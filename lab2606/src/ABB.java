...
public class ABB<K, V> implements IMapeamento<K, V>{
    ...
    private V pesquisar(No<K, V> raizArvore, K procurado) {
    	...
    	if (raizArvore == null)
    		...
    		throw new NoSuchElementException("O item não foi localizado na árvore! Chave: "+procurado);
    ...
    public V obterPorIndice(int i) {
        int[] contador = {0};
        return obterPorIndiceRecursivo(raiz, i, contador);
    }
    
    private V obterPorIndiceRecursivo(No<K, V> no, int alvo, int[] contador) {
        if (no == null) return null;
    
        V resultado = obterPorIndiceRecursivo(no.getEsquerda(), alvo, contador);
        if (resultado != null) return resultado;
    
        if (contador[0] == alvo) return no.getItem();
        contador[0]++;
    
        return obterPorIndiceRecursivo(no.getDireita(), alvo, contador);
    }
